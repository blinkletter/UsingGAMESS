...
# 
# ***GAMESS***[^gamess1],[^gamess2],[^gamess_link]  is an ab initio computation package for investigating molecular structure and energy. There are others, such as ***NWChem*** [^nwchem],[^nwchem_link]  and ***ORCA*** [^orca],[^orca_link]. ***MacMolPlt*** [^macmolplt],[^macmolplt_link]  is a front end for GAMESS. It can build molecules, export input files and interpret the output files produced by GAMESS. ***Avogadro*** [^avogadro],[^avogadro_link]  is a more modern option but lacks some useful tools compared to MacMolPlt. There are others, such as ***Gabedit*** [^gabedit],[^gabedit_link]  and ***Molden***. [^molden],[^molden_link] 
# 
...
# #### Text Editor
# 
# We will need a text editor. We could use *TextEdit*, which comes with *MacOS*. I will be using *BBEdit*, which has a useful free version available for *MacOS*.[^bbedit_link] 
# 
# #### Terminal Emulator
# 
...
# [^bbedit_link]: https://www.barebones.com/products/bbedit/
